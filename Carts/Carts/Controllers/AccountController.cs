...
namespace Carts.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            //透過輸入email登入
            //UserName改為儲存暱稱，所以要先透過傳入的Email取得UserName
            //目前沒有發現 SignInManager.PasswordSignInAsync 有其他多載，只能用UserName登入
            string email = model.Email; //傳入的email
            string userName; 
            using(UserEntities db = new UserEntities())
            {
                userName = (from s in db.AspNetUsers where s.Email == email select s.UserName).FirstOrDefault();
            }
